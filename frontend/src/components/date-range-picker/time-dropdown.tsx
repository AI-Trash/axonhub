  ...
  return (
    ...
    >
      ...
      {onClose && (
        <button
          type='button'
          className='absolute -top-8 right-0 text-[11px] font-semibold tracking-widest text-gray-400 uppercase hover:text-gray-200'
          onClick={onClose}
        >
          {closeLabel}
        </button>
...
function TimeCol({ label, items, active, onPick }: { label: string; items: number[]; active: string; onPick: (val: string) => void }) {
  return (
