...
export function TimeField({ label, value, active, open, onToggle, onChange, onClose, closeLabel, wrapperRef }: TimeFieldProps) {
  const inputClass = cn(
    'w-full border-none bg-transparent p-0 text-sm transition-colors focus:ring-0 focus:outline-none',
    active ? 'font-semibold text-gray-900 dark:text-gray-100' : 'font-medium text-gray-500 dark:text-gray-600'
  );

  const iconClass = cn('ml-2 h-5 w-5 transition-colors', active ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400 dark:text-gray-600');
