    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive flex items-center gap-2'>
          ...
          {m["channels.dialogs.bulkDelete.title"]()}
        </span>
      }
      desc={m["channels.dialogs.bulkDelete.description"]({ count: selectedCount })}
    >
      <div className='flex items-start gap-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm dark:border-red-900 dark:bg-red-900/20'>
        ...
        <div className='space-y-1 text-left'>
          <p className='font-semibold text-red-900 dark:text-red-100'>{m["channels.dialogs.bulkDelete.warning"]()}</p>
          <p className='text-red-800 dark:text-red-200'>{m["channels.dialogs.bulkDelete.warningDetail"]()}</p>
        </div>
      </div>
    </ConfirmDialog>
  );
