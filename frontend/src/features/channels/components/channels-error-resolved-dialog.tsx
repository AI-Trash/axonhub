  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-[425px]'>
        <DialogHeader>
          <DialogTitle>{m["channels.dialogs.errorResolved.title"]()}</DialogTitle>
          <DialogDescription>
            {t('channels.dialogs.errorResolved.description', {
              channelName: currentRow.name,
              errorMessage: dynamicTranslation(`channels.messages.${currentRow.errorMessage}`),
            })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='py-4'>
          <div className='bg-muted rounded-md p-3'>
            <p className='text-sm font-medium'>{m["channels.dialogs.errorResolved.currentError"]()}</p>
            <p className='text-muted-foreground mt-1 text-sm'>
              {dynamicTranslation(`channels.messages.${currentRow.errorMessage}`)}
            </p>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={updateChannelErrorMessage.isPending}>
            {updateChannelErrorMessage.isPending ? m["common.buttons.processing"]() : m["channels.actions.errorResolved"]()}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
