...
async function createFilesystemDataStorage(page: Page) {
  ...
  const submitButton = dialog.getByRole('button', { name: /创建|Create|保存|Save/i }).filter({ hasText: /创建|Create|保存|Save/i });

  await Promise.all([waitForGraphQLOperation(page, 'CreateDataStorage'), submitButton.first().click()]);
...
test.describe('Admin Data Storage Management', () => {
  ...
  test('can create and edit a filesystem data storage', async ({ page }) => {
    ...
    await Promise.all([waitForGraphQLOperation(page, 'UpdateDataStorage'), saveButton.click()]);
  ...
  test('can create and edit an S3 data storage', async ({ page }) => {
    ...
    if ((await typeSelector.count()) > 0) {
      ...
      const s3Option = page.getByRole('option', { name: /S3/i }).or(page.locator('[data-value="s3"]'));
  ...
  test('can create and edit a GCS data storage', async ({ page }) => {
    ...
    if ((await typeSelector.count()) > 0) {
      await typeSelector.first().click();
      const gcsOption = page.getByRole('option', { name: /GCS|Google/i }).or(page.locator('[data-value="gcs"]'));
  ...
  test('can archive a data storage from the actions menu', async ({ page }) => {
    ...
    await Promise.all([waitForGraphQLOperation(page, 'UpdateDataStorageStatus'), confirmButton.click()]);
