...
test.describe('Admin System Management', () => {
  ...
  test('can view system tabs and update brand settings', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /System|系统/i }).first()).toBeVisible();
    ...
    await Promise.all([waitForGraphQLOperation(page, 'UpdateBrandSettings'), saveButton.click()]);
    ...
    if (newValue !== originalValue) {
      ...
      await Promise.all([waitForGraphQLOperation(page, 'UpdateBrandSettings'), revertButton.click()]);
