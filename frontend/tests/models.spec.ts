...
test.describe('Admin Models Management', () => {
  ...
  test('can create, edit, filter, toggle status, and delete a model', async ({ page }) => {
    ...
    // Open create dialog
    const createButton = page.getByRole('button', { name: /Add Model|创建模型|新增模型|创建|Add/i }).first();
    ...
    const confirmStatusButton = statusDialog.getByRole('button', { name: /Confirm|确认|确定|Enable|Disable/i }).last();
    await Promise.all([waitForGraphQLOperation(page, 'UpdateModel'), confirmStatusButton.click()]);
    ...
    await Promise.all([waitForGraphQLOperation(page, 'DeleteModel'), deleteButton.click()]);
