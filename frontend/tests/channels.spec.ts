...
test.describe('Admin Channels Management', () => {
  ...
  test('can filter channels by type', async ({ page }) => {
    ...
    const typeFilterButton = page.getByRole('button', { name: /Type|类型/i }).or(page.locator('button').filter({ hasText: /Type|类型/i }));
    ...
    const openaiFilter = page.getByRole('option', { name: /OpenAI/i }).or(page.locator('[role="option"]').filter({ hasText: /OpenAI/i }));
  ...
  test('can navigate between pages', async ({ page }) => {
    ...
    const pagination = page.locator('[data-testid="pagination"]').or(page.locator('nav').filter({ hasText: /页|Page|Previous|Next/i }));
  ...
  test('can configure model mappings in model mapping dialog', async ({ page }) => {
    ...
    await Promise.all([waitForGraphQLOperation(page, 'UpdateChannel'), saveButton.click()]);
