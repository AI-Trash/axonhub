...
export async function signInAsAdmin(page: Page, credentials: AdminCredentials = defaultCredentials) {
  ...
  const emailField = page.getByTestId('sign-in-email').or(page.locator('input[type="email"], input[name="email"]')).first();

  await emailField.waitFor({ state: 'visible', timeout: 20000 });

  // Fill in credentials with test IDs and fallback selectors
  const passwordField = page.getByTestId('sign-in-password').or(page.locator('input[type="password"], input[name="password"]')).first();
  ...
  const responsePromise = page.waitForResponse((response) => response.url().includes('/admin/auth/signin') && response.status() === 200, {
    timeout: 15000,
  });
...
export async function waitForGraphQLOperation(page: Page, operationName: string) {
  const lowerCamel = operationName.length ? operationName.charAt(0).toLowerCase() + operationName.slice(1) : operationName;
  try {
    await Promise.race([
