...
> **重要：** Rust 后端已验证 **SQLite 和 PostgreSQL** 支持。MySQL 通过相同的 SeaORM 支持切片已布线，但完整的 Rust 端自动化集成验证仍在进行中。TiDB 与 Neon DB 相关内容仅保留在旧 Go 树中作为历史参考，不是此仓库的 canonical 部署路径。详见 [后端架构](#后端架构) 部分。
...
使用官方 Helm Chart 在 Kubernetes 上部署 AxonHub。此部署路径使用 Rust 后端作为 canonical 镜像，支持 PostgreSQL（已验证）和 MySQL（已布线但未完全验证）。TiDB 与 Neon DB 相关材料仍保留在旧 Go 树中作为历史参考；这里记录的 Helm 路径仅面向 Rust canonical 镜像。
