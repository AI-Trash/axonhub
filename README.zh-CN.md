...
- **管理 GraphQL**: `POST /admin/graphql` 含 playground，以及当前受支持的 canonical 子集：设置管理与选定的用户管理写操作；更广泛的管理写能力仍在扩展中
- **OpenAPI GraphQL**: `POST /openapi/v1/graphql` 含 playground
- **OpenAI 兼容 `/v1` 推理（仅标准 JSON 请求）**: `/models`, `/chat/completions`, `/responses`, `/embeddings`, `/messages`, `/rerank`, `/images/generations`，以及仅支持 JSON 的 `POST /v1/realtime`
...
超出上述已验证范围的路由族，仍然会由 Rust 侧返回显式 `501 Not Implemented`。具体包括：

- **图像编辑及其余图像变体**: `POST /v1/images/edits` 与其他尚未迁移的图像路由，仍然落在 Rust 显式 `501 Not Implemented` 边界上
- **实时 API**: 仅支持 JSON `POST /v1/realtime` 走标准 Rust OpenAI `/v1` 执行路径；专门的 realtime WebSocket 传输、upgrade 风格请求以及 session 系列路由仍保持显式 `501 Not Implemented` 边界
- **更广泛的管理后台写能力**: 项目/角色管理、配额配置、备份/配额检查/GC cleanup 等运维 mutation，以及超出当前 canonical「设置管理 + 选定用户管理」子集之外的其他写操作
...
对于上述**受支持产品能力面**，请使用 Rust 二进制、`ghcr.io/looplj/axonhub:rust-latest` 或 `docker-compose.rust.yml`。

对于上面列出的显式未支持边界，请预期 Rust 侧返回真实的 `501 Not Implemented` 响应，而不是切换到 Go fallback。旧 Go 树仅作为历史参考保留在仓库中。
...
这些交付物会保留 Rust CLI / 配置契约，并交付当前 Rust 测试矩阵已验证的 SQLite 与 PostgreSQL 替代能力面：`/health`、admin bootstrap/status/auth/read 流程、canonical `/admin/graphql` 当前受支持子集（包含已验证的设置写操作 mutation，以及当前扩展中的选定用户管理切片）、OpenAPI GraphQL、request-context/auth 基础能力，以及已迁移的 inference 路由族（包含 `POST /v1/images/generations` 与仅支持 JSON 的 `POST /v1/realtime`）。专门的 realtime WebSocket/session 传输及任何超出该受支持范围的路由族，仍会由 Rust 返回显式 `501 Not Implemented`。
