...
AxonHub 的 canonical 后端实现已经固定为 Rust。Rust workspace 与 Rust 标记的发布交付物，构成仓库当前对外的主部署路径。

### 当前 Rust 支持能力面（已验证）

Rust workspace 与带 Rust 标记的发布交付物，提供完整且经过验证的功能，覆盖 SQLite 和 PostgreSQL 部署：
...
- **管理 GraphQL**: `POST /admin/graphql` 含 playground，完整支持设置管理、用户管理、项目/角色管理、配额配置、运维操作（备份、恢复、GC cleanup 等）
- **OpenAPI GraphQL**: `POST /openapi/v1/graphql` 含 playground
- **OpenAI 兼容 `/v1` 推理**: `/models`, `/chat/completions`, `/responses`, `/responses/compact`, `/embeddings`, `/messages`, `/rerank`, `/images/generations`, `/images/edits`, `/images/variations`, `/v1/realtime`（JSON POST）、realtime WebSocket 升级与会话管理（`/v1/realtime/sessions`）、视频生成
- **其他提供商 API**: Jina、Anthropic、Gemini、Doubao 等路由（见 routes 文件）
- **AiSDK 兼容性**: 完整支持 Vercel AI SDK 协议，通过 `X-Vercel-Ai-Ui-Message-Stream` 和 `X-Vercel-AI-Data-Stream` 请求头
- **Provider-edge 管理 OAuth**: Codex、Claude Code、Antigravity、Copilot
- **RBAC 与权限**: 细粒度访问控制，支持系统级和项目级 scopes
- **核心业务逻辑**: 渠道/模型关联与获取、用量/成本追踪、追踪/线程管理、系统初始化
- **Transformer/Pipeline**: 提供商编排、出站转换器、中间件管道
- **企业级功能**: 提示词保护、提供商配额管理、熔断器
- **配置**: 完整对齐 AxonHub 配置选项（SQLite/PostgreSQL）
- **数据库支持**: SQLite 和 PostgreSQL（canonical）；MySQL/TiDB/Neon 不在 Rust 后端支持范围内

### 仅作为历史参考

`cmd/axonhub/main.go`、`conf/conf.go` 与 `internal/server/**` 下的旧 Go 树，仍保留在仓库中作为契约 / oracle 历史与实现参考。它们并不是当前 AxonHub 的 canonical 构建、发布或部署路径。

### 实际含义

上述**受支持产品能力面**由 Rust 后端完整提供。请使用 Rust 二进制、`ghcr.io/looplj/axonhub:rust-latest` 或 `docker-compose.rust.yml`。

Rust workspace 是 canonical 后端实现；旧 Go 树仅作为历史参考保留在仓库中，不是部署或回退路径。
...
| **图片生成（Image Generation）** | ✅ Done | 图像生成、编辑、变体 | [Image Generation](docs/zh/api-reference/image-generation.md) |
...
| **实时对话（Realtime）** | ✅ Done | 实时对话能力（WebSocket、会话） | [OpenAI API](docs/zh/api-reference/openai-api.md) |
...
这些交付物会保留 Rust CLI / 配置契约，并交付当前 Rust 测试矩阵已验证的完整 SQLite 与 PostgreSQL 能力面：`/health`、admin bootstrap/status、identity/request-context、admin read routes、完整的 `/admin/graphql`（settings、user management、project/role management、quota、operational mutations）、OpenAPI GraphQL，以及完整的 inference 路由族（`/v1/images/generations`、`/v1/images/edits`、`/v1/images/variations`、`/v1/realtime` JSON POST 与 WebSocket 升级+会话管理、video generation，以及 Jina、Anthropic、Gemini、Doubao 等提供商路由）。AiSDK 兼容性与 provider-edge OAuth 也已完全支持。
...
> **重要：** Rust 后端已验证 **SQLite 和 PostgreSQL** 支持。MySQL 不属于此仓库中 Rust 目标态支持契约的一部分。TiDB 与 Neon DB 相关内容仅保留在旧 Go 树中作为历史参考，不是此仓库的 canonical 部署路径。详见 [后端架构](#后端架构) 部分。
...
| 数据库 | 支持版本 | 推荐场景 | 自动迁移 | 链接 |
| -------- | ------------------ | -------------------- | -------------- | ------ |
| **TiDB Cloud** | Starter | Serverless, Free tier, Auto Scale | ✅ 支持（仅 Go） | [TiDB Cloud](https://www.pingcap.com/tidb-cloud-starter/) |
| **TiDB Cloud** | Dedicated | 分布式部署、大规模 | ✅ 支持（仅 Go） | [TiDB Cloud](https://www.pingcap.com/tidb-cloud-dedicated/) |
| **TiDB** | V8.0+ | 分布式部署、大规模 | ✅ 支持（仅 Go） | [TiDB](https://tidb.io/) |
| **Neon DB** | - | Serverless, Free tier, Auto Scale | ✅ 支持（仅 Go） | [Neon DB](https://neon.com/) |

这些条目仅保留为旧 Go 契约能力面的历史参考；当前仓库中的 canonical 部署指导仍以 Rust 后端为准。

#### 配置文件 | Configuration

**Rust 后端（SQLite）：**

AxonHub 使用 YAML 配置文件，支持环境变量覆盖。Rust 后端默认使用 SQLite，无需额外配置。
...
**历史参考（旧 Go 多 Dialect 示例）：**

TiDB 与 Neon DB 的配置示例仍保留在旧 Go 树与文档中，仅作为历史参考材料。当前仓库里的 canonical 部署指导仍以 Rust 后端及其已明确说明的受支持能力面为准。
...
TiDB 与 Neon DB 的 compose 示例仍保留在旧 Go 树与文档中，仅作为历史参考材料。本部署指南保持 Rust 为 canonical 操作路径。

#### Helm Kubernetes 部署 | Helm Kubernetes Deployment

使用官方 Helm Chart 在 Kubernetes 上部署 AxonHub。此部署路径使用 Rust 后端作为 canonical 镜像，并以 PostgreSQL 作为已验证的 Kubernetes 数据库路径。TiDB 与 Neon DB 相关材料仍保留在旧 Go 树中作为历史参考；这里记录的 Helm 路径仅面向 Rust canonical 镜像。
...
TiDB 与 Neon DB 的虚拟机示例仍保留在旧 Go 树中，仅作为历史参考材料。此 README 保持 Rust 标记资产为 canonical 发布路径。
...
- 🦀 [tokio-rs/axum](https://github.com/tokio-rs/axum) - Rust 后端早期阶段使用的 HTTP 框架
- ⚙️ [actix-rs/actix-web](https://github.com/actix-rs/actix-web) - Rust 后端生产环境使用的 HTTP 框架
- ⚙️ [tokio-rs/tokio](https://github.com/tokio-rs/tokio) - Rust 后端使用的异步运行时
