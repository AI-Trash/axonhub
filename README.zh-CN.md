...
- **管理 GraphQL**: `POST /admin/graphql` 含 playground，以及当前受支持的设置管理子集
...
- **更广泛的管理后台写能力**: 用户/项目/角色管理、配额配置，以及超出当前 canonical 设置管理子集之外的其他写操作
...
这些交付物会保留 Rust CLI / 配置契约，并交付当前 Rust 测试矩阵已验证的 SQLite 与 PostgreSQL 替代能力面：`/health`、admin bootstrap/status/auth/read 流程、canonical `/admin/graphql` 当前受支持子集（包含已验证的设置写操作 mutation）、OpenAPI GraphQL、request-context/auth 基础能力，以及已迁移的 inference 路由族（包含 `POST /v1/images/generations`）。任何超出该受支持范围的路由族，仍会由 Rust 返回显式 `501 Not Implemented`。
